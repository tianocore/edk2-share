...
def parseCmdArgs():
    ...
    parser.add_option('-d', '--define', action='append', dest='Macros',
                      help='Specify the macro used in preprocess package\'s source. For example: -d __GNUC__')
    ...
    return (options.WorkspacePath, options.PackagePath, options.DoxygenPath, options.OutputPath,
            options.Macros, options.Arch, options.DocumentMode, options.IncludeOnly, options.HtmlWorkshopPath)
...
if __name__ == '__main__':
    wspath, pkgpath, doxpath, outpath, macros, archtag, docmode, isinc, hwpath = parseCmdArgs()
